...
private extension AuthController {
    ...
    func existingUser(matchingTokenString tokenString: RefreshToken.Token,on connection: DatabaseConnectable) throws -> Future<LoginUser?> {
        return RefreshToken.query(on: connection).filter(\.tokenString == tokenString).first().flatMap({ (token) in
            ...
            return LoginUser.query(on: connection).filter(\.userID == token.userID).first()
        })
    }
    ...
    func removeAllTokens(for user:LoginUser,on connection: DatabaseConnectable) throws -> Future<Void> {
        let accessTokens = AccessToken.query(on: connection).filter(\.userID == user.userID!).delete()
        let refreshToken = RefreshToken.query(on: connection).filter(\.userID == user.userID!).delete()
